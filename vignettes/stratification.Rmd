...
vignette: >
  %\VignetteIndexEntry{Custom Stratification}
...
Second, make sure you have the BBS data downloaded.
```{r, eval = FALSE}
fetch_bbs_data()
```
...
## Subsettin an existing stratification
...
s <- stratify(by = "bbs_cws", strata_custom = my_cws)
```
...
```{r}
s <- stratify(by = "WBPHS_2019", strata_custom = map)
```
...
Let's see what that looks like by comparing to the raw BBS data.
...
So we can clearly see, spatially, which points have been omitted from our data.

Okay, let's continue.


```{r}
p <- prepare_data(data_strat, species = "Western Meadowlark", min_year = 2000)
...
```{r}
i <- generate_indices(m, regions = "stratum")
